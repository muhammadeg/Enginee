...
void IChatbox::HookCommand()
{
	...
	Tools->Intercept(ITools::_I_CALL, (void*)0x006CB655, Hooks::KCommand::ProcessCommand, 5);
	Tools->Intercept(ITools::_I_CALL, (void*)0x0077C163, Hooks::KCommand::ProcessCommand, 5);
}
...
int IChatbox::EnterText(std::string Text)
{
	...
	if (Text[0] == '/')
	{
		...
		else if (c == "commandlist")
		{
			Chatbox->Display("   [Server Commands List]");
			Chatbox->Display("   /skinoff");
			Chatbox->Display("   /skinon");
			Chatbox->Display("   /mpetoff");
			Chatbox->Display("   /mpeton");
			Chatbox->Display("   /geonoff");
			Chatbox->Display("   /geonon");
			Chatbox->Display("   /absorboff");
			Chatbox->Display("   /absorbon");
